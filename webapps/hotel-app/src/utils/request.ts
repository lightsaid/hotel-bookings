...
class Request {
    ...
    // Get 请求
    public Get<T>(url: string, config: AxiosRequestConfig, toastConfig: ToastConfig = defaultToastConfig): Promise<T> {
        // TODO: 处理 catch
    ...
    // Post 请求
    public Post<T>(url: string, config: AxiosRequestConfig, toastConfig: ToastConfig = defaultToastConfig): Promise<T> {
        return this.instance.post(url, config.data, config).then(res => {
            return this.handleRes(res, toastConfig)
        })
        .catch((error) => {
